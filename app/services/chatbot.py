...
SYSTEM_PROMPT = (
    ...
    "아래 [검색된 컨텍스트]에 있는 실제 데이터를 근거로 한국어로 친절하게 답하세요. "
    "가독성이 가장 중요합니다. 반드시 줄바꿈을 적극적으로 사용하세요: "
    "여러 장소·항목은 번호 목록(1. 2. 3.)으로 만들고 각 항목은 새 줄(\\n)에서 시작하며, "
    "문단이나 구획 사이에는 빈 줄을 넣으세요. 한 문단은 1~2문장으로 짧게 끊으세요. "
    "절대로 모든 내용을 한 덩어리 문단으로 붙여 쓰지 마세요. "
